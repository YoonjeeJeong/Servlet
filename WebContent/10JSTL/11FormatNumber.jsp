...
<h3>jstl국제화 태그로 출력</h3>
money : <fmt:formatNumber value="${money }"/> -> 10000

<!-- groupingUsed : false값이면 컴마 생략됨 -->
<h3>JSTL국제화 태그로 출력</h3>
money : <fmt:formatNumber groupingUsed="false" value="${money }" />

<h3>type="currency"이고 currencySymbol생략시 - 현재통화 단위로 출력됨</h3>
money : <fmt:formatNumber value="${money}" type="currency" /><br>
money : <fmt:formatNumber value="${money}" type="currency" currencySymbol="$" />

<!-- value속성값으로 전달되는 값을 %단위로 변환해서 출력한다. -->
<h3>type="percent"</h3>
money : <fmt:formatNumber value="0.02" type="percent" currencySymbol="$" />

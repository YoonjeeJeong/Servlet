...
try{
	...
	//사용자(User-Agent)의 웹브라우저 정보를 반환
	client = request.getHeader("User-Agent");

	//파일 다운로드 헤더 지정
	/*
	파일 다운로드를 위한 응답헤더 설정
	: 웹 브라우저가 인식하지 못하는 컨텐츠타입(MIME)을
	응답헤더에 설정하면 다운로드 창을 보여준다.
	(웹브라우저에 따라 다운로드 창의 형태는 달라짐)
	*/
	response.reset() ;
	...
	if(!skip){
		...
		if(client.indexOf("MSIE") != -1){
			//웹브라우저의 종류가 익스플로러(MSIE)일때 한글파일명 처리
		...
		//이때 웹 브라우저는 인식하지 못하므로 다운로드창을 통해 다운로드 시킴.

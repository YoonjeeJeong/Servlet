...
저장할 수 있다. object타입으로 자동 형변환되어 저장되고, 사용시에는
각 객체로 형변환후 사용하면 된다. 차후EL(표현언어)을 사용하면 형변환없이
사용할 수 있다.

1. page영역
	-페이지 영역에 저장된 속성은 해당 페이지에서만 공유되고, 페이지를
	벗어나는 순간 소멸된다.
	-페이지영역에는 pageContext 내장객체를 사용하여 영역을
	사용한다.
*/
...
<body>
   ...
   <%
   /*
   page영역에 저장될 때 object타입으로 자동현변환되어 저장되므로
   읽어올때도 object타입으로 가져오는 것이 기본이다.
   */
   ...
   <%--페이지 영역에 저장된 속성값은 새로운 페이지에 대한 요청이 발생하면 속성값이 모두 파괴되어 소멸된다.
   즉 다른 페이지와는 속성이 공유되지 않는다. --%>
   <h2>페이지 이동</h2>
   <a href="PageResult.jsp">page영역 공유확인을 위한 링크</a>
   <br />
   <%--
   외부파일을 인클루드 하는 경우 외부문서는 원본그대로 현재문서에 포함된 후
   컴파일 되었으므로 include지시어를 사용하는 경우 같은 페이지로 취급되어
   페이지 영역이 공유된다.
    --%>

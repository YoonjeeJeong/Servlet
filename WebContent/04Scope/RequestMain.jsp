...
<body>
	...
	<%
		request.removeAttribute("NoKeyValue");//키 없으나 에러발생 없으
	...
	URL주소창에는 최초 요청된 페이지의 경로명이 보여지지만, 출력되는 페이지는 새롭게
	...
	dis.forward(request, response); 포워드 사용시 영역이 파괴되지 않는다*/
	%>
	...
	response.sendRedirect("RequestResult.jsp?param="+
						URLEncoder.encode("리다이렉트방식", "UTF-8"));//영역은 데이터를 저장하는 공간이다.
	%>

...
폼값을 받을 수 있다. 만약 값이 입력되지 않았다면 길이가 0인 스트링 객체를 
...
if(!(name==null || name.equals("")) ){
	...
	아래와같이 사용할 수 있다. 또는 문자열을 연결하여 사용할수도 
	있다. 
	*/
	out.println("<script>");
...
항목은 여러개이나, 실제 선택 가능한 항목은 하나이기 때문이다.
*/
//방법1: (아래) text타입과 동일한 형태로 폼값을 받을때
...
if(favo != null){
	for(int i=0 ; i<favo.length ; i++) {
		/*
		전송된 항목을 하나의 문자열로 연결하여 변수로 저장한다.
...
웹브라우저에 출력할때는 특수기호를 <br>태그로 반환 후 출력해야 한다.
*/
String self_intro = 
...
getParameterNames() : 폼값의 이름을 한꺼번에 받을때 사용하는 
		request 내장객체의 매소드로 반환타입은 Enumeration형이다
...
while(names.hasMoreElements()){
	...
	if(paramName.equals("sex")||paramName.equals("favorite")){
		...
		System.out.println("파라미터명을 메소드로 얻어서 값 출력");

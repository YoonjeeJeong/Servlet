...
응답헤더 설정하기
	...
	이는 캐시에 저장된 내용을 출력하고 있기때문이다.
	아래설정은 캐시를 사용하지 않고 매번 새로고침(f5)한것처럼
	새로운 정보로 갱신하겠다는 응답헤더 설정이다.
*/
//HTTP/1.0에서 적용할 때 사용(현재는 사용하지 않으나 호환성을 위해 남겨둠)
...
/* 파일다운로드 구현시 사용(아래)
	...
	파일 다운로드창을 통해 파일을 다운로드 시켜버린다.
	JPG와 같은 이미지파일은 웹 브라우저가 인식하는 MIME타입이므로
	다운로드를 위해서는 아래와 같은 설정이 필요하다<div class=""></div>*/
//response.setContentType("binary/octect-stream");


/*응답헤더 추가/설정하기
	add계열: 기존의 세더명이 존재할 경우 기존의 값을 유지하면서
...
response.addIntHeader("numberOfCrew", 99);//기존 헤더 덮어쓰기(수정) ->99만 출력
%>    
<!DOCTYPE html>
<html>
<head>

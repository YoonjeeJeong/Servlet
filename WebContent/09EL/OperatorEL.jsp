...
<body>
	...
	<h3>EL에서의 null연산</h3>
	<%/*
	//자바코드에서는 null과 연산을 수행할 수 없다.
	하지만 EL에서는 null을 0으로 간주하여 계산한다.
	따라서 NullPointerException이 발생 안 함
	int a = null + 10;*/%>
	...
	만약 
		해당 페이지?myNumber=20인 경우 20+10 => 결과 30이 출력된다.
		해당 페이지?myNumber= 인 경우 0으로 간주되어 10출력됨
		해당 페이지?myNumber=삼 인 경우 숫자로 변경불가하므로 에러발생
	 -->
	...
	<%
	/*
	EL에서는 jsp에서 선언한 변수는 직접 사용불가다. 값은 출력되지 않고
	null로 인식하게 된다. jsp의 변수를 el에서 사용할 수 없는 이유는
	el은 4가지 영역에 저장된 속성들만 사용하기 때문이다. jstl도 동일한 특성을 가지고 있다.*/
	String varScriptLet = "스크립렛 안에서 변수선언";%>
	...
	<!-- 톰캣8.0부터는 el에서 변수할당이 가능해졌다. 하지만 개발시에는
	...
	\${fnum=99 } : ${fnum=99 }<!-- 99로 재할당 되어 99출력됨 -->
	
	<!-- 
	el에서는 정수와 정수를 연산하더라도 '실수'의 결과가 나올 있다.
	즉 자동형변화되어 출력된다.
	...
	스트링클래스의 compareTo()메소드의와 같은 방식으로 비교한다.
	...
	<ul>
		<li>\${fnum > snum} : ${fnum>snum }</li><!-- 문자열로서 비교하게 된다. 그래서 false. -->
		<li>\${100 >90} : ${100 > 90 } </li>
		<li>\${"JAVA"=="JAVA"} : ${"JAVA" == "JAVA"}  </li>
		<li>\${"Java"=="JAVA"} : ${"Java"=="JAVA" } </li>		
		<!-- 자바에서는 문자열을 비교할 떄 equals()메소드로 통해 비교하지만
		el에서는 == 형태로 배교한다. -->
	</ul>
	
	<h3>el의 논리연산자</h3>
	<ul>
 		<li>\${5>=5 && 10!=10} : ${5 ge 5 && 10 ne 10}</li><!-- 참&&거짓 =>거짓 -->
		<li>\${5>6 || 10<100} : ${5>6 || 10 lt 100}</li><!-- 거짓||참 =>참 -->
	</ul>
	...
	<!-- null 이거나 ""(빈문자열)일때
	혹은 배열인 경우 길이가 0일때 혹은 컬렉션인 경우 size가 0일때 
	true를 반환하는 연산자이다. -->
	...
	<ul>
		...
		<li>${empty ellengthZero ? "배열크기0임" : "배열크기 0아님"}</li>

...
<body>
	<fieldset>
		...
		<!-- 
		  el은 null에 비해 어느정도까지는 처리를 해주므로 에러가 발생하지 않는다
		 -->		
		...
		<ul>
			<li>\${elArray[0] } : ${elArray[0]}</li>
			<li>\${elCollection[0] } : ${elCollection[0] }</li>
		<%-- 	<li>\${elArray.0 } : \${elArray.0 } [500에러]</li>
			<li>${elCollection.0 } : \${elCollection.0 } [500에러]</li> --%>
		</ul>
		
		<!-- jstl의 forEach태그를 통해 반복출력. 해당문장은 배열이나
		배연의 크기만큼 반복하는 확장 for문의 형태로 기술됨. -->
		...
		<ul>
			<li>존재하지 않는 속성: \${eldto.addr } : \${eldto.addr } \[PropertyNotFountException발생]</li>
			<li>속성은 존재, getter가 없는 경우 : \${eldto.id } : \${eldto.id } [500에러 발생]</li>
			<li>el변수명이 틀린경우 : \${noeldto.pw } : \${noeldto.pwd } [에러no, 미출력]</li>
		</ul>
		
		<h2>EL 내장 객체의 없는 속성으로 접근시</h2>
		<ul>
			<li>\${requestScope.nop } : ${requestScope.nop }
				[Scope계열 내장객체, 에러 노, 미출력 ->속성명은 개발자 마음대로 정할 수 있음]</li>
			<!-- 
		...
		<ul>
			<li>\${requestScope.reqProperty } : ${requestScope.reqProperty } [정상]</li>
			<li>\${requestScope.REQProperty } : ${requestScope.REQProperty } [에러, 미출력]</li>
			<li>\${elMap.mapKey} : ${elMap.mapKey} </li>
			<li>\${elMap.MAPKey } : ${elMap.MAPKey } [에러, 미출력]</li>
		</ul>
		
		<h2>숫자를 0으로 나누면 INFINITE</h2>
		\${10 div 0} : ${10 div 0}
		<h2>EL변수에 값 할당시 톰캿8.0부터는 정상. 이전버전은 에러.</h2> 

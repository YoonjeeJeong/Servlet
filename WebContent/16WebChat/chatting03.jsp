...
//접속자아이디를 가져오는 부분(현재는 랜덤하게 생성되는 세션아이디)
var chat_id = document.getElementById("chat_id").value;

//웹소켓 객체 생성(서버에 접속)
var webSocket = new WebSocket("ws://localhost:8282/K07JSPServlet/ChatServer02");//집에서 포트와 폴더 확인할 것
//위에건 선생님의 포트: 192.168.219.118:8282/JSPServlet/ChatServer02
...
//웹소켓 서버가 메세지를 받은 후 클라이언트에게 Echo할 때...
function wsMessage(event){
	
	var message = event.data.split("|");//메세지를 | 구분자로 split한다
	var sender = message[0];//메세지의 첫번째 부분은 전송한사람의 아이디
	var content = message[1];//두번째 부분은 메세지
	if (content == ""){
		//전송한 메세지가 없다면 아무 일도 하지 않는다		
	}
	else {
		//보낸 메세지에... 
		if(content.match("/")){
			// 슬래시(/)가 포함되어 있다면 명령어로 인식
...
function sendMessage(){
	...
	*/
	...
	//입력했던 대화내용을 지워준다(안 그럼 입력창 매번 지워야 함)
	inputMessage.value = "";
}
function disconnect(){
webSocket.close();

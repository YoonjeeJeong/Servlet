...
public class FileUtil {
	...
	public static MultipartRequest upload(HttpServletRequest req, String saveDirectory) {
		...
		/*
		 매개변수 1: 사용자 요청 정보가 저장된 request 내장객체(req)
		 매개변수 2: 파일이 업로드 될 서버의 물리적 경로(saveDirectory)
		 그 외 업로드할 파일의 최대용량, 인코딩방식, 파일명 중복을 처리할 클래스를
		 통해 MultipartRequest객체를 생성한다.
		 객체 생성이 정상적으로 되면 파일 업로드는 완료된다
		 */
		try {
	...
	public static void deleteFile(HttpServletRequest req, String directory, String filename) {
		...
		//물리적 경로를 통해 파일객체 생성
		java.io.File f = new java.io.File(saveDirectory+java.io.File.separator+filename);
	...
	public static void download(
						...
						String directory, String ofileName, String sfileName){
